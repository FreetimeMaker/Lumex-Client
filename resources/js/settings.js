...
export class SettingsService {
  async getResolvedPaths() {
    ...
    let dataRoot;
    if (flatpakId) {
      dataRoot = join(home, '.var', 'app', 'com.freetime.lumexclient', 'data', 'lumex-client');
    } else if (isWindows) {
      dataRoot = join(appData || home, 'LumexClient');
    } else {
      dataRoot = join(xdgData || join(home, '.local', 'share'), 'lumex-client');
    }

    return {
      platform: isWindows ? 'windows' : 'linux',
      flatpak: Boolean(flatpakId),
      dataRoot,
      instancesDir: join(dataRoot, 'instances'),
  ...
  async defaults() {
    ...
    return {
      maxMemoryMb: 4096,
      javaPath: '',
      minecraftDataDir: join(paths.dataRoot, 'minecraft'),
  ...
  async save(settings) {
    await Neutralino.storage.setData(STORAGE_KEY, JSON.stringify(settings));
  }
